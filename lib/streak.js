...
// Fetch every logged day for the user, sorted oldest -> newest
export async function fetchAllDays() {
  ...
  if (error) {
    console.error('fetchAllDays error:', error);
    return [];
  }

  // log_date comes back as 'YYYY-MM-DD' string; convert to local Date at midnight
  return data.map(row => ({
